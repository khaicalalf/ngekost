...
@section('content')
    <div id="Background"
        ...
        <div id="TopNav" class="relative flex items-center justify-between px-5 mt-[60px]">
            ...
            <p class="font-semibold">Browse Ngekost</p>
            <div class="dummy-btn w-12"></div>
        </div>
        <div id="Header" class="relative flex items-center justify-between gap-2 px-5 mt-[18px]">
            <div class="flex flex-col gap-[6px]">
                <h1 class="font-bold text-[32px] leading-[48px]">in {{ $categories->name }}</h1>
                <p class="text-ngekos-grey">Tersedia {{ $categories->boardingHouses->count() }} {{ $categories->name }}</p>
            </div>
        ...
        @foreach ($boardingHouses as $boardingHouse)
            <a href="{{ route('kost.show', $popularBoardingHouse->slug) }}" class="card">
                <div
